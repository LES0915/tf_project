...
resource "aws_lb" "public" {
  ...
  # 퍼블릭 서브넷 2개 (가용영역별 a, c) 각각 id를 추출(리스트 컴프리헨션 방식 문법) 반영
...
resource "aws_lb" "internal" {
  ...
  # APP 프라이빗 서브넷 2개 (가용영역별 a, c) 각각 id를 추출(리스트 컴프리헨션 방식 문법) 반영
  subnets = [for subnet in aws_subnet.app : subnet.id]
  tags    = { Name = "${local.project}-INTERNAL-ALB" }
}
# was 직접 점검 => 8000번 점속, 모두 상위 설정값과 동일
