...
resource "aws_autoscaling_group" "web" {
  ...
  # 서버구성 후 180초 동안 헬스 체크 실패는 무시(최초 구성 시 로드, 업데이트 등 정상 x)
  health_check_grace_period = 180

  # EC2를 launch_template 이용하여 구성하겠다 (상세 구성 내용-런치 템플릿)
  launch_template {
  ...
  # 내용변경 (버전변경) -> 교체(순차)
  instance_refresh {
    # 장애/ 업그레이드 등 이슈로 교체 시 -> 순차적 처리 -> (1개 신규 생성 -> 1개 삭제..)
    strategy = "Rolling"
    # 순차 교체 세부 조건
    preferences {
      # 최소 50% 성능 유지 (헬스 체크 등)
...
resource "aws_autoscaling_group" "was" {
  ...
  health_check_type         = "ELB"
  # 240초 종안 헬스 체크 오류는 무시 was가 구성 상 시간 더 소모됨
