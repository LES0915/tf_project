  ...
  security_groups = {
    ...
    was = {
      ingress = {
        ...
        # 오직 web -> was로만 접근해야 함, 취지 벗어남 (다른 방식으로 구성)
        # tcp   = {
        #     port = 8000,
        #     cidr = ["0.0.0.0/0"]
        # }
    ...
    db = {
      ingress = {
        ...
        # 오직 was -> db로만 접근해야 함, 취지 벗어남 (다른 방식으로 구성)
        # tcp   = {
        #     port = 3306,
        #     cidr = ["0.0.0.0/0"]
        # }
...
resource "aws_security_group" "sg" {
  ...
  # 이름 => DE-AI-13-web-sg-해시값, DE-AI-13-was-sg-해시값, DE-AI-13-db-sg-해시값
  name_prefix = "DE-AI-13-${each.key}-sg-"
  description = "${upper(each.key)} Security Group"

  # VPC 지정
  vpc_id = aws_vpc.DE-AI-13-company.id

  # ingress 동적 생성 => 블럭 반복 구성 => dynamic ingress
  dynamic "ingress" {
