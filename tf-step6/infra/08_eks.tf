...
# 쿠버네티스 컨트롤 플레인 생성, EKS Auto Mode 기능 활성화
resource "aws_eks_cluster" "main" {
  ...
  # EKS Control plane이 AWS 리소스 관리할 때 IAM role
  role_arn = aws_iam_role.eks_cluster.arn

  # Auto Mode(자동 직접 관리) 이므로 컨트롤 플레인에 필요한 기능 등 별도로 addon 하지 않음
  ...
  compute_config {
    ...
    # 노드 구성 => 노드풀에서 가져와서 구성
    node_pools = [
      "general-purpose", # web, was 등 pod 실행하는 용도의 node
      "system"            # 중요 시스템 pod용
    ]

    # auto mode가 node_pools에서 타입에 맞춰 용도에 맞는 node 생성(ec2 만듦) -> IAM Role 적용
...
# ────────────────────────────────────────────────
# Metrics Server addon 구성 (CPU 사용량 등 => pod 증감 등 관련 지표)
# ────────────────────────────────────────────────
...
# ────────────────────────────────────────────────
# IAM Role 부분 추가 등록 등 처리 - 추가 관리자 access 엔트리
# ────────────────────────────────────────────────
